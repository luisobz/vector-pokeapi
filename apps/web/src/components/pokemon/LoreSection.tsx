...
export default function LoreSection({ description }: LoreSectionProps) {
  const t = useTranslations("PokemonDetail");
  const [isExpanded, setIsExpanded] = useState(false);

  if (!description) return null;

  // Split description by new lines to create paragraphs
    ...
    <section className="space-y-4">
      ...
      <div className="relative p-6 pb-18 rounded-2xl bg-zinc-900/50 border border-white/5">
        ...
        {!isExpanded && paragraphs.length > 0 && (
          <div className="absolute bottom-14 left-0 right-0 h-18 from-zinc-900/90 to-transparent pointer-events-none rounded-b-2xl" />
        )}
